...
public class ChunkedHashStore<T> implements Serializable, SafelyCloseable, Iterable<ChunkedHashStore.Chunk> {
    ...
    public ChunkedHashStore(final TransformationStrategy<? super T> transform, final File tempDir, final int hashWidthOrCountValues, final ProgressLogger pl) throws IOException {
        this(transform, tempDir, hashWidthOrCountValues, pl, new ByteBufferPool());
    }
    ...
    public ChunkedHashStore(final TransformationStrategy<? super T> transform, final File tempDir, final int hashWidthOrCountValues, final ProgressLogger pl, final ByteBufferPool byteBufferPool) throws IOException {
        ...
        for (int i = 0; i < DISK_CHUNKS; i++) {
            byteBuffer[i] = acquireByteBuffer();
    ...
    private ByteBuffer acquireByteBuffer() {
        final ByteBuffer buffer = this.byteBufferPool.acquire(BUFFER_SIZE);
        return buffer.order(ByteOrder.nativeOrder());
    }

    private void releaseByteBuffer(final ByteBuffer buffer) {
    ...
    public long size() throws IOException {
        ...
        if (filteredSize == -1) {
            long c = 0;
            final long[] triple = new long[3];
            final ByteBuffer iteratorByteBuffer = acquireByteBuffer();
            for (int i = 0; i < DISK_CHUNKS; i++) {
                if (filter == null) c += count[i];
                else {
                    flushAll();
                    @SuppressWarnings("resource") final ReadableByteChannel channel = new FileInputStream(file[i]).getChannel();
            ...
            releaseByteBuffer(iteratorByteBuffer);
    ...
    public void close() throws IOException {
        if (!closed) {
            ...
            for (final ByteBuffer b : byteBuffer) releaseByteBuffer(b);
        }
    }
    ...
    public Iterator<Chunk> iterator() {
        ...
        return new ObjectIterator<Chunk>() {
            private final ByteBuffer iteratorByteBuffer = acquireByteBuffer();
